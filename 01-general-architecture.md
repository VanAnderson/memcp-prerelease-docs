...
Understanding this architecture starts with `MCPServer` - the central orchestrator that coordinates transports, sessions, and provider systems into a cohesive MCP-compliant server that can serve any MCP client.
...
constructor(config: MCPServerConfig, toolRegistry: ToolRegistry, logger: Logger) {
  ...
  this.setupSessionManagerIntegration();   // → Sets up session lifecycle events
}
```
...
async function createProductionServer(configPath: string): Promise<MCPServer> {
  ...
  const serverConfig: MCPServerConfig = {
    ...
    session: { sessionTimeout: 300000, maxSessions: 1000 }
  };
...
new MCPServer(serverConfig, toolRegistry, logger)
  ...
  → MCPServer#setupSessionManagerIntegration()  // Session event handlers
